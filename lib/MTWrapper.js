     ...
     if (uri != null) {
       ...
       if ('db' in options && options.db != '*' && (!isRestore || isLegacyRestoreDb())) {// keep legacy capability
         ...
         if (isOptionSet(options, 'collection')) {
          command += ' --nsInclude ' + options.db + '.' + options.collection;
         } else {
          command += ' --nsInclude ' + options.db + '.*';
          if ('targetDb' in options && options.targetDb && options.db !== options.targetDb) {
           command += ' --nsFrom ' + options.db + '.*';
           command += ' --nsTo ' + options.targetDb + '.*';
          }
         }
       }
     }
